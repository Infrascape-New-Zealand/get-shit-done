---
name: iscape:graphify
description: "Build, query, and inspect the project knowledge graph in context/graphs/"
...
**CJS-only (graphify):** `graphify` subcommands are not registered on `iscape-sdk query`. Use `node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `iscape-sdk query` where a handler exists.
...
Check if graphify is enabled by reading `context/config.json` directly using the Read tool.

**DO NOT use the iscape-tools config get-value command** -- it hard-exits on missing keys.

1. Read `context/config.json` using the Read tool
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs config-set graphify.enabled true

Then run /iscape-graphify build to create the initial graph.
```
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /iscape-graphify build to create or rebuild the graph.`
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs graphify status
```
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" graphify build)
```
...
Project root: ${CWD}
iscape-tools path: $HOME/.claude/get-shit-done/bin/iscape-tools.cjs
...
2. **Validate output:**
   ...
   Include the stderr output for debugging. Do NOT delete context/graphs/ -- prior valid graph remains available.

3. **Copy artifacts to context/graphs/:**
   ```
   cp graphify-out/graph.json context/graphs/graph.json
   cp graphify-out/graph.html context/graphs/graph.html
   cp graphify-out/GRAPH_REPORT.md context/graphs/GRAPH_REPORT.md
   ```
   These three files are the build output consumed by query, status, and diff commands.

4. **Write diff snapshot:**
   ```
   node \"$HOME/.claude/get-shit-done/bin/iscape-tools.cjs\" graphify build snapshot
   ```
   This creates context/graphs/.last-build-snapshot.json for future diff comparisons.
...
4. DO NOT use iscape-tools config get-value for the config gate -- it exits on missing keys
