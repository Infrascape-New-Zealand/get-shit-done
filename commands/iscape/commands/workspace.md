---
name: iscape:workspace
description: Manage Iscape workspaces — create, list, or remove isolated workspace environments
...
<objective>
Manage Iscape workspaces with a single consolidated command.

Mode routing:
- **--new**: Create an isolated workspace with repo copies and independent context/ → new-workspace workflow
- **--list**: List active Iscape workspaces and their status → list-workspaces workflow
- **--remove**: Remove a Iscape workspace and clean up worktrees → remove-workspace workflow
</objective>
...
| --list | Scan ~/iscape-workspaces/, show summary table | list-workspaces |
