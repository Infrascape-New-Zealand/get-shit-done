---
name: iscape:config
description: Configure Iscape settings — workflow toggles, advanced knobs, integrations, and model profile
...
<objective>
Configure Iscape settings interactively with a single consolidated command.
...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | iscape-sdk config-set-model-profile |

</routing>

<execution_context>
@./workflows/settings.md
@./workflows/settings-advanced.md
@./workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. **Pre-flight check (#2439):** verify `iscape-sdk` is on PATH via `command -v iscape-sdk`.
     If absent, emit the install hint `Install Iscape via 'npm i -g get-shit-done'` and stop —
     do NOT invoke `iscape-sdk` directly (avoids the opaque `command not found: iscape-sdk` failure).
  2. Run: `iscape-sdk query config-set-model-profile <profile-name> --raw` and display the output verbatim.
