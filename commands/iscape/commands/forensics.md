---
type: prompt
name: iscape:forensics
description: Post-mortem investigation for failed Iscape workflows — analyzes git history, artifacts, and state to diagnose what went wrong
...
<objective>
Investigate what went wrong during a Iscape workflow execution. Analyzes git history, `context/` artifacts, and file system state to detect anomalies and generate a structured diagnostic report.

Purpose: Diagnose failed or stuck workflows so the user can understand root cause and take corrective action.
Output: Forensic report saved to `context/forensics/`, presented inline, with optional issue creation.
</objective>
...
- `context/STATE.md` (current position, session history)
- `context/ROADMAP.md` (phase scope and progress)
- `context/phases/*/` (PLAN.md, SUMMARY.md, VERIFICATION.md, CONTEXT.md)
- `context/reports/SESSION_REPORT.md` (last session outcomes)
...
- Structured forensic report written to `context/forensics/report-{timestamp}.md`
