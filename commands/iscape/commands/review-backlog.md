...
1. **List backlog items:**
   ```bash
   ls -d context/phases/999* 2>/dev/null || echo "No backlog items found"
   ```
...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" phase add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs: review backlog — promoted N, removed M" --files context/ROADMAP.md
   ```
