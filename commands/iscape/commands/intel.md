---
name: iscape:intel
description: "Query, inspect, or refresh codebase intelligence files in context/intel/"
...
Check if intel is enabled by reading `context/config.json` directly using the Read tool.
...
Intel system is disabled. To activate:
  ...
  node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs config-set intel.enabled true

Then run /iscape:intel refresh to build the initial index.
```
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs intel query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If no matches found, display: `No intel matches for '<term>'. Try /iscape:intel refresh to build the index.`
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs intel status
```
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs intel diff
```
...
```
Iscape > Spawning intel-updater agent to analyze codebase...
```
...
Task(
  ...
  prompt="You are the iscape-intel-updater agent. Your job is to analyze this codebase and write/update intelligence files in context/intel/.
...
2. Write JSON intel files to context/intel/ (stack.json, api-map.json, dependency-graph.json, file-roles.json, arch-decisions.json)
3. Each file must have a _meta object with updated_at timestamp
4. Use iscape-tools intel extract-exports <file> to analyze source files
5. Use iscape-tools intel patch-meta <file> to update timestamps after writing
6. Use iscape-tools intel validate to check your output
...
4. DO NOT use the iscape-tools config get-value CLI for the config gate -- it exits on missing keys
