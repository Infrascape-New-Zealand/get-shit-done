...
```bash
phase_dir="context/phases/${phase_num}-${slug}"
mkdir -p "$phase_dir"
```
...
2. Insert new phase heading:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /iscape:plan-phase {N} to break down)
...
- Don't use decimal numbering (that's /iscape:insert-phase)
- Don't create plans yet (that's /iscape:plan-phase)
...
- [ ] Phase directory created: `context/phases/{NN}-{slug}/`
