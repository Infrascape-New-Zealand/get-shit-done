...
Template for project-root `CLAUDE.md` — auto-generated by `iscape-tools generate-claude-md`.
...
**Fallback text:**
```
Project not yet initialized. Run /iscape:new-project to set up.
```
...
## Iscape Workflow Enforcement

Before using Edit, Write, or other file-changing tools, start work through a Iscape command so planning artifacts and execution context stay in sync.

Use these entry points:
- `/iscape:quick` for small fixes, doc updates, and ad-hoc tasks
- `/iscape:debug` for investigation and bug fixing
- `/iscape:execute-phase` for planned phase work

Do not make direct repo edits outside a Iscape workflow unless the user explicitly asks to bypass it.
<!-- GSD:workflow-end -->
```
...
> Profile not yet configured. Run `/iscape:profile-user` to generate your developer profile.
...
5. **Workflow Enforcement** — Default Iscape entry points for file-changing work
