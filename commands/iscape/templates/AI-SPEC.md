...
> AI design contract generated by `/iscape-ai-integration-phase`. Consumed by `iscape-planner` and `iscape-eval-auditor`.
...
> Researched by `iscape-domain-researcher`. Grounds the evaluation strategy in domain expert knowledge.
...
> Fetched from official docs by `iscape-ai-researcher`. Distilled for this specific use case.
...
> Written by `iscape-ai-researcher`. Cross-cutting patterns every developer building AI systems needs — independent of framework choice.
