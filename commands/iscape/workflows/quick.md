<purpose>
Execute small, ad-hoc tasks with Iscape guarantees (atomic commits, STATE.md tracking). Quick mode spawns iscape-planner (quick mode) + iscape-executor(s), tracks tasks in `context/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node ./bin/iscape-tools.cjs init quick "$DESCRIPTION")
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/iscape:new-project` first.
...
```
Task(
  prompt="First, read ~/.claude/agents/iscape-planner.md for your role and instructions.\n\n" + revision_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
Spawn iscape-executor with plan reference:
...
```bash
node ./bin/iscape-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
