<purpose>
Generate a post-session summary document capturing work performed, outcomes achieved, and estimated resource usage. Writes SESSION_REPORT.md to context/reports/ for human review and stakeholder sharing.
</purpose>
...
Read `context/STATE.md` to get:
- Current milestone and phase
- Progress percentage
- Active blockers
- Recent decisions

Read `context/ROADMAP.md` to get milestone name and goals.

Check for existing reports:
```bash
ls -la context/reports/SESSION_REPORT*.md 2>/dev/null || echo "No previous reports"
```
</step>
...
Write `context/reports/SESSION_REPORT.md` (or `context/reports/YYYYMMDD-session-report.md` if previous reports exist):

```markdown
# Iscape Session Report
...
*Generated by `/iscape:session-report`*
```
</step>
...
```
💡 Run `/iscape:session-report` at the end of each session to build a history of project activity.
```
</step>
...
- [ ] Report written to context/reports/
