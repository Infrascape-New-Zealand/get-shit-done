...
```bash
INIT=$(node ./bin/iscape-tools.cjs init execute-phase "${PHASE}")
```
...
# config settings can be fetched via iscape-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="iscape-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
if [ ! -f context/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > context/agent-history.json
fi
rm -f context/current-agent-id.txt
if [ -f context/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat context/current-agent-id.txt)
...
2. Per segment:
   - Subagent route: spawn iscape-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node ./bin/iscape-tools.cjs phases list --type summaries --raw
...
See ./references/checkpoints.md for details.
</step>
...
```bash
grep -A 50 "^user_setup:" context/phases/XX-name/{phase}-{plan}-PLAN.md | head -50
```

If user_setup exists: create `{phase}-USER-SETUP.md` using template `./templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `context/phases/XX-name/`. Use `./templates/summary.md`.
...
Update STATE.md using iscape-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ./bin/iscape-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node ./bin/iscape-tools.cjs state update-progress

# Record execution metrics
node ./bin/iscape-tools.cjs state record-metric \
...
node ./bin/iscape-tools.cjs state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node ./bin/iscape-tools.cjs state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using iscape-tools:

```bash
node ./bin/iscape-tools.cjs state record-session \
...
```bash
node ./bin/iscape-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ./bin/iscape-tools.cjs requirements mark-complete ${REQ_IDS}
```
...
```bash
node ./bin/iscape-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files context/phases/XX-name/{phase}-{plan}-SUMMARY.md context/STATE.md context/ROADMAP.md context/REQUIREMENTS.md
```
</step>

<step name="update_codebase_map">
If context/codebase/ doesn't exist: skip.
...
```bash
node ./bin/iscape-tools.cjs commit "" --files context/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/iscape:execute-phase {phase}` + `/iscape:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/iscape:plan-phase {Z+1}` + `/iscape:verify-work {Z}` + `/iscape:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/iscape:complete-milestone` + `/iscape:verify-work` + `/iscape:add-phase` |
