...
Then read:
1. `context/PROJECT.md` (first 80 lines — project context)
2. Phase section from `context/ROADMAP.md`
3. All `*-PLAN.md` files in the phase directory
4. `*-CONTEXT.md` if present (user decisions)
5. `*-RESEARCH.md` if present (domain research)
6. `context/REQUIREMENTS.md` (requirements this phase addresses)
</step>
...
Write to a temp file: `/tmp/iscape-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/iscape-review-prompt-{phase}.md)" 2>/dev/null > /tmp/iscape-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/iscape-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/iscape-review-claude-{phase}.md
```

**Codex:**
```bash
codex -p "$(cat /tmp/iscape-review-prompt-{phase}.md)" 2>/dev/null > /tmp/iscape-review-codex-{phase}.md
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► CROSS-AI REVIEW — Phase {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Commit:
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/iscape:plan-phase --reviews)
</success_criteria>
