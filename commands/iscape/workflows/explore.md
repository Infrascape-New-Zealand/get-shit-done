...
offers mid-conversation research when useful, then routes crystallized outputs to Iscape artifacts.
</purpose>
...
@./references/domain-probes.md
</required_reading>

<available_agent_types>
Valid Iscape subagent types (use exact names — do not fall back to 'general-purpose'):
- iscape-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
|------|-------------|-----------------|
| Note | `context/notes/{slug}.md` | Observations, context, decisions worth remembering |
| Todo | `context/todos/pending/{slug}.md` | Concrete actionable tasks identified |
| Seed | `context/seeds/{slug}.md` | Forward-looking ideas with trigger conditions |
| Research question | `context/research/questions.md` (append) | Open questions that need deeper investigation |
...
- **Notes:** Create `context/notes/{slug}.md` with frontmatter (title, date, context)
- **Todos:** Create `context/todos/pending/{slug}.md` with frontmatter (title, date, priority)
- **Seeds:** Create `context/seeds/{slug}.md` with frontmatter (title, trigger_condition, planted_date)
- **Research questions:** Append to `context/research/questions.md`
- **Requirements:** Append to `context/REQUIREMENTS.md` with next available REQ ID
- **Phases:** Use existing `/iscape:add-phase` command via SlashCommand

Commit if `commit_docs` is enabled:
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/iscape:explore` or start working with `/iscape:next`.
```
