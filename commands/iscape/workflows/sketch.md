...
Each sketch produces 2-3 variants for comparison. Saves artifacts to `context/sketches/`.
Companion to `/iscape-sketch-wrap-up`.
...
If no `context/sketches/` directory exists, tell the user there's nothing to analyze and offer to start fresh with an idea instead.
...
**c. All sketch READMEs** — read `context/sketches/*/README.md` for design questions, winners, and tags.
...
Present all consistency and frontier candidates, then ask which to run. When the user picks sketches, update `context/sketches/MANIFEST.md` and proceed directly to building them starting at `build_sketches`.
</step>

<step name="setup_directory">
Create `context/sketches/` and themes directory if they don't exist:

```bash
mkdir -p context/sketches/themes
```

Check for existing sketches to determine numbering:
```bash
ls -d context/sketches/[0-9][0-9][0-9]-* 2>/dev/null | sort | tail -1
```
...
**b.** Read `context/spikes/MANIFEST.md` if it exists — check the Requirements section for non-negotiable design constraints (e.g., "must support streaming", "must render markdown"). These requirements should be visible in the mockup even though the mockup doesn't implement them for real.

**c.** Read `context/spikes/CONVENTIONS.md` if it exists — the established stack informs what's buildable and what interaction patterns are idiomatic.
...
Create or update `context/sketches/MANIFEST.md`:
...
If no theme exists yet at `context/sketches/themes/default.css`, create one based on the mood/direction from the intake step. See `sketch-theme-system.md` for the full template.
...
**b.** Create the sketch directory: `context/sketches/NNN-descriptive-name/`
...
## How to View
open context/sketches/NNN-descriptive-name/index.html
...
Open: `open context/sketches/NNN-name/index.html`
...
3. Update `context/sketches/MANIFEST.md`

**h.** Commit (if `COMMIT_DOCS` is true):
```bash
iscape-sdk query commit "docs(sketch-NNN): [winning direction] — [key visual insight]" --files context/sketches/NNN-descriptive-name/ context/sketches/MANIFEST.md
```
...
`/iscape-sketch-wrap-up`

───────────────────────────────────────────────────────────────

**Also available:**
- `/iscape-sketch` — sketch more (or run with no argument for frontier mode)
- `/iscape-plan-phase` — start building the real UI
- `/iscape-spike` — spike technical feasibility of a design pattern
...
<success_criteria>
- [ ] `context/sketches/` created (auto-creates if needed, no project init required)
