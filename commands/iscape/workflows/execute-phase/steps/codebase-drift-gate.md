...
```bash
DRIFT=$(iscape-sdk query verify.codebase-drift 2>/dev/null || echo '{"skipped":true,"reason":"sdk-failed"}')
```
...
Run /iscape-map-codebase --paths {affected_paths} to refresh planning context.
```
...
from step `init_context` is for `iscape-executor`, not the mapper):

```bash
AGENT_SKILLS_MAPPER=$(iscape-sdk query agent-skills iscape-codebase-mapper)
```

Then spawn `iscape-codebase-mapper` agents with the `--paths` hint:
