# sync-skills — Cross-Runtime Iscape Skill Sync

**Command:** `/iscape-sync-skills`

Sync managed `iscape-*` skill directories from one canonical runtime's skills root to one or more destination runtime skills roots. Keeps multi-runtime installs aligned after a `iscape-update` on one runtime.
...
# If running from a global install, resolve relative to the Iscape package
...
error: source skills root not found: <path>
       Is Iscape installed globally for the '<runtime>' runtime?
       Run: node ./bin/install.js --global --<runtime>
```
Then exit.
...
```bash
# List iscape-* subdirectories in source
SRC_SKILLS=$(ls -1 "$SRC_SKILLS_ROOT" 2>/dev/null | grep '^iscape-')

# List iscape-* subdirectories in destination (may not exist yet)
DST_SKILLS=$(ls -1 "$DEST_ROOT" 2>/dev/null | grep '^iscape-')

# Diff:
# CREATE  — in SRC but not in DST
# UPDATE  — in both; content differs (compare recursively via checksums)
# REMOVE  — in DST but not in SRC (stale Iscape skill no longer in source)
# SKIP    — in both; content identical (already up to date)
```

**Non-Iscape preservation:** Only `iscape-*` entries are ever created, updated, or removed. Entries in the destination that do not start with `iscape-` are never touched.
...
CREATE: iscape-help
UPDATE: iscape-update
REMOVE: iscape-old-command
SKIP:   iscape-plan-phase (up to date)
(N changes)
...
1. **Only `iscape-*` directories** are created, updated, or removed. Any directory not starting with `iscape-` in a destination root is untouched.
2. **Dry-run is the default.** `--apply` must be passed explicitly to write anything.
3. **Source root must exist.** Never create the source root; it must have been created by a prior `iscape-update` or installer run.
...
- Sync copies files verbatim and does not apply runtime-specific content transformations. Use the Iscape installer directly for runtimes that require format conversion.
