...
```bash
ls context/phases/XX-current/*-PLAN.md 2>/dev/null | sort
ls context/phases/XX-current/*-SUMMARY.md 2>/dev/null | sort
```
...
```bash
ls context/phases/XX-current/.continue-here*.md 2>/dev/null
```
...
```bash
cat context/phases/XX-current/*-SUMMARY.md
```
...
Exit skill and invoke SlashCommand("/iscape:plan-phase [X+1]")
...
**Also available:**
- `/iscape:discuss-phase [X+1]` — gather context first
- `/iscape:research-phase [X+1]` — investigate unknowns
- Review roadmap
...
Exit skill and invoke SlashCommand("/iscape:complete-milestone {version}")
