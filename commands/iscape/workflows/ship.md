...
```bash
CONFIG=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files context/STATE.md
```
</step>
...
- /iscape:complete-milestone (if last phase in milestone)
- /iscape:progress (to see what's next)
...
- /iscape:complete-milestone — if all phases in milestone are done
- /iscape:progress — see overall project state
- /iscape:execute-phase {next} — continue to next phase
</offer_next>
