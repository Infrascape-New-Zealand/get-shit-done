...
Seeds auto-surface during /iscape:new-milestone when trigger conditions match the
...
EXISTING=$(ls context/seeds/SEED-*.md 2>/dev/null | wc -l)
NEXT=$((EXISTING + 1))
...
Write `context/seeds/SEED-{PADDED}-{slug}.md`:
...
This seed should be presented during `/iscape:new-milestone` when the milestone
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs: plant seed — {$IDEA}" --files context/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
Trigger: {$TRIGGER}
Scope: {$SCOPE}
File: context/seeds/SEED-{PADDED}-{slug}.md

This seed will surface automatically when you run /iscape:new-milestone
...
<success_criteria>
- [ ] Seed file created in context/seeds/
