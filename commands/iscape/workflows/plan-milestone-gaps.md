<purpose>
Create all phases necessary to close gaps identified by `/iscape:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/iscape:add-phase` per gap.
</purpose>
...
PHASES=$(node ./bin/iscape-tools.cjs phases list)
...
grep -c "Pending" context/REQUIREMENTS.md
```
...
```bash
node ./bin/iscape-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files context/ROADMAP.md context/REQUIREMENTS.md
```
...
**Also available:**
- `/iscape:execute-phase {N}` — if plans already exist
