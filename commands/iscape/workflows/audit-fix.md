...
- iscape-executor — executes a specific, scoped code change
</available_agent_types>
...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" init audit-uat 2>/dev/null || echo "{}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Read existing UAT and verification files to extract findings:
- Glob: `context/phases/*/*-UAT.md`
