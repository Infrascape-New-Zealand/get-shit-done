<purpose>
Generate an AI design contract (AI-SPEC.md) for phases that involve building AI systems. Orchestrates iscape-framework-selector → iscape-ai-researcher → iscape-domain-researcher → iscape-eval-planner with a validation gate. Inserts between discuss-phase and plan-phase in the Iscape lifecycle.
...
```bash
INIT=$(iscape-sdk query init.plan-phase "$PHASE")
...
```bash
SELECTOR_MODEL=$(iscape-sdk query resolve-model iscape-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(iscape-sdk query resolve-model iscape-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(iscape-sdk query resolve-model iscape-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(iscape-sdk query resolve-model iscape-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(iscape-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /iscape-settings.
```
Exit workflow.

**If `planning_exists` is false:** Error — run `/iscape-new-project` first.
...
```bash
PHASE_INFO=$(iscape-sdk query roadmap.get-phase "${PHASE}")
```
...
Recommended: run /iscape-discuss-phase {N} first to capture framework preferences.
...
## 5. Spawn iscape-framework-selector

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► AI DESIGN CONTRACT — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Step 1/4 — Framework Selection...
```

Spawn `iscape-framework-selector` with:
```markdown
Read ~/.claude/agents/iscape-framework-selector.md for instructions.
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /iscape-ai-integration-phase {N} or answer the framework question in /iscape-discuss-phase {N} first."
...
Spawn `iscape-ai-researcher` with:
```markdown
Read ~/.claude/agents/iscape-ai-researcher.md for instructions.
...
Spawn `iscape-eval-planner` with:
```markdown
Read ~/.claude/agents/iscape-eval-planner.md for instructions.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► AI-SPEC COMPLETE — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Next step:
  /iscape-plan-phase {N}   — planner will consume AI-SPEC.md
```
