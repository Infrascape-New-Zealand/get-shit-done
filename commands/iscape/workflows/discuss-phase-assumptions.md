...
- iscape-assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>

<downstream_awareness>
**CONTEXT.md feeds into:**

1. **iscape-phase-researcher** — Reads CONTEXT.md to know WHAT to research
2. **iscape-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" agent-skills iscape-assumptions-analyzer 2>/dev/null)
```
...
Use /iscape:progress to see available phases.
```
Exit workflow.
...
```bash
cat context/PROJECT.md 2>/dev/null || true
cat context/REQUIREMENTS.md 2>/dev/null || true
cat context/STATE.md 2>/dev/null || true
```
...
```bash
(find context/phases -name "*-CONTEXT.md" 2>/dev/null || true) | sort
```
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
```bash
cat context/METHODOLOGY.md 2>/dev/null || true
```
...
- When spawning the iscape-assumptions-analyzer, pass the lens list so it can flag which lenses apply
...
```bash
ls context/codebase/*.md 2>/dev/null || true
```
...
Spawn a `iscape-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files context/STATE.md
```
</step>
...
```
Created: context/phases/${PADDED_PHASE}-${SLUG}/${PADDED_PHASE}-CONTEXT.md
...
**Also available:**
- `/iscape:plan-phase ${PHASE} --skip-research` — plan without research
- `/iscape:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
2. Sync chain flag:
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```

**If `--auto` flag present AND `AUTO_CHAIN` is not true:**
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-set workflow._auto_chain_active true
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► AUTO-ADVANCING TO PLAN
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Context captured (assumptions mode). Launching plan-phase...
```

Launch: `Skill(skill="iscape:plan-phase", args="${PHASE} --auto")`
