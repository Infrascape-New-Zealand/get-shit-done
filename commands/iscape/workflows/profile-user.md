...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing iscape-tools.cjs subcommands and the iscape-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
- @$HOME/.claude/get-shit-done/agents/iscape-user-profiler.md (profiler agent definition)
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /iscape:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn iscape-user-profiler agent using Task tool:**

Use the Task tool to spawn the `iscape-user-profiler` agent. Provide it with:
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/iscape-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/iscape-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```
...
- options (ALL pre-selected by default):
  - "/iscape:dev-preferences command file" -- "Load your preferences in any session"
...
**For /iscape:dev-preferences (if selected):**

```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /iscape:dev-preferences at $HOME/.claude/commands/iscape/commands/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/get-shit-done/bin/iscape-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any iscape-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```
Artifacts:
  ✓ /iscape:dev-preferences   $HOME/.claude/commands/iscape/commands/dev-preferences.md
...
- [ ] Artifacts generated sequentially via iscape-tools.cjs subcommands
