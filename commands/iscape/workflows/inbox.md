...
Write this report to `context/INBOX-TRIAGE.md` if a `context/` directory exists,
...
```bash
gh issue close {number} --comment "Closed by Iscape inbox triage: this issue is missing required fields per the issue template. Missing: {list}. Please reopen with a complete submission. See CONTRIBUTING.md for requirements."
```

For PRs with gate violations:
```bash
gh pr close {number} --comment "Closed by Iscape inbox triage: this PR does not meet the issue-first requirement. {specific violation}. See CONTRIBUTING.md for the correct process."
```
...
{If report saved: "Report saved to context/INBOX-TRIAGE.md"}
...
- /iscape:review — Run cross-AI peer review on a specific phase plan
- /iscape:ship — Create a PR from completed work
- /iscape:progress — See overall project state
- /iscape:inbox --label — Re-run with auto-labeling enabled
</offer_next>
