...
- **Project scope**: `context/notes/{YYYY-MM-DD}-{slug}.md` — used when `context/` exists in cwd
- **Global scope**: `~/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `context/`, or when `--global` flag is present
...
**`--global` flag**: Strip `--global` from anywhere in `$ARGUMENTS` before parsing. When present, force global scope regardless of whether `context/` exists.

**Important**: Do NOT create `context/` if it doesn't exist. Fall back to global scope silently.
</step>
...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/iscape:note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
2. Ensure the notes directory exists (`context/notes/` or `~/.claude/notes/`)
...
1. Glob `context/notes/*.md` (if directory exists) — project notes
...
{count} active note(s). Use `/iscape:note promote <N>` to convert to a todo.
```
...
4. **Requires `context/` directory** — if it doesn't exist, warn: "Todos require a Iscape project. Run `/iscape:new-project` to initialize one."
5. Ensure `context/todos/pending/` directory exists
6. Generate todo ID: `{NNN}-{slug}` where NNN is the next sequential number (scan both `context/todos/pending/` and `context/todos/done/` for the highest existing number, increment by 1, zero-pad to 3 digits) and slug is the first ~4 meaningful words of the note text
7. Extract the note text from the source file (body after frontmatter)
8. Create `context/todos/pending/{id}.md`:
...
status: pending
priority: P2
source: "promoted from /iscape:note"
created: {YYYY-MM-DD}
theme: general
---
...
<edge_cases>
1. **"list" as note text**: `/iscape:note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `context/`**: Falls back to global `~/.claude/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `context/`, suggests `/iscape:new-project`
...
- [ ] Global fallback: Works when no `context/` exists
</success_criteria>
