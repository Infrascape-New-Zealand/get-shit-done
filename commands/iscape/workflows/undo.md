<purpose>
Safe git revert workflow. Rolls back Iscape phase or plan commits using the phase manifest with dependency checks and a confirmation gate. Uses git revert --no-commit (NEVER git reset) to preserve history.
</purpose>
...
```
Usage: /iscape:undo --last N | --phase NN | --plan NN-MM

Modes:
  --last N      Show last N Iscape commits for interactive selection
  --phase NN    Revert all commits for phase NN
  --plan NN-MM  Revert all commits for plan NN-MM

Examples:
  /iscape:undo --last 5
  /iscape:undo --phase 03
  /iscape:undo --plan 03-02
```
</step>
...
Filter for Iscape conventional commits matching `type(scope): message` pattern (e.g., `feat(04-01):`, `docs(03):`, `fix(02-03):`).

Display a numbered list of matching commits:
```
Recent Iscape commits:
...
Read `context/.phase-manifest.json` if it exists.
...
Read `context/ROADMAP.md` inline.
...
1. Check if `context/phases/${N}-*/` directory exists
...
Look for later plans in the same phase directory (`context/phases/${NN}-*/`). For each later plan (plans with number > MM):
...
```
Working tree has uncommitted changes. Commit or stash them before running /iscape:undo.
```
...
**Also available:**
- `/iscape:execute-phase ${PHASE}` — re-execute if needed
- `/iscape:undo --last 1` — undo the revert itself if something went wrong
...
- [ ] --phase mode reads context/.phase-manifest.json using manifest.phases[TARGET_PHASE].commits
