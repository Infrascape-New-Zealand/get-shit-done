...
- iscape-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" agent-skills iscape-security-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" resolve-model iscape-security-auditor --raw)
SECURITY_CFG=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
```

If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /iscape:settings."

Display banner: `Iscape > SECURE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /iscape:execute-phase {N} first."
...
## 5. Spawn iscape-security-auditor

```
Task(
  prompt="Read ~/.claude/agents/iscape-security-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `./templates/SECURITY.md`
...
```
Iscape > PHASE {N} SECURITY BLOCKED
{K} threats open — phase advancement blocked until threats_open: 0
▶ Fix mitigations then re-run: /iscape:secure-phase {N}
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
```
Iscape > PHASE {N} THREAT-SECURE
threats_open: 0 — all threats have dispositions.
▶ /iscape:validate-phase {N}    validate test coverage
▶ /iscape:verify-work {N}       run UAT
```
