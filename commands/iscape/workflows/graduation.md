...
```bash
GRADUATION_ENABLED=$(iscape-sdk query config-get features.graduation 2>/dev/null || echo "true")
GRADUATION_WINDOW=$(iscape-sdk query config-get features.graduation_window 2>/dev/null || echo "5")
GRADUATION_THRESHOLD=$(iscape-sdk query config-get features.graduation_threshold 2>/dev/null || echo "3")
```
...
```bash
find context/phases -name "*-LEARNINGS.md" | sort | tail -n "$GRADUATION_WINDOW"
```
...
Read `context/STATE.md` `graduation_backlog` section (if present). Format:
...
Write to `context/STATE.md` under `graduation_backlog`:
```yaml
- cluster_id: "{sha256}"
  status: "deferred"
