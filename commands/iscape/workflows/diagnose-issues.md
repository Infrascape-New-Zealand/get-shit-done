...
Debug files use the `context/debug/` path (hidden directory with leading dot).
</paths>
...
- truth: "Comment appears immediately after submission"
  ...
  debug_session: context/debug/comment-not-refreshing.md
```
...
```bash
node ./bin/iscape-tools.cjs commit "docs({phase_num}): add root causes from diagnosis" --files "context/phases/XX-name/{phase_num}-UAT.md"
```
</step>
