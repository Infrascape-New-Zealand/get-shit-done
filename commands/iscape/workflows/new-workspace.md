<purpose>
Create an isolated workspace directory with git repo copies (worktrees or clones) and an independent `context/` directory. Supports multi-repo orchestration and single-repo feature branch isolation.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /iscape:new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /iscape:new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /iscape:new-project    # Initialize Iscape in the workspace
```
...
**Offer to initialize Iscape (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize Iscape"
- question: "Would you like to initialize a Iscape project in the new workspace?"
- options:
  - "Yes — run /iscape:new-project" → tell user to `cd $TARGET_PATH` first, then run `/iscape:new-project`
...
- [ ] `context/` directory initialized at workspace root
