<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns iscape-code-reviewer agent, commits REVIEW.md, and presents results to user.
</purpose>
...
- iscape-code-reviewer: Reviews source files for bugs and quality issues
</available_agent_types>
...
```bash
PHASE_ARG="${1}"
INIT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" init phase-op "${PHASE_ARG}")
...
```
Error: Phase ${PHASE_ARG} not found. Run /iscape:progress to see available phases.
```
...
```bash
CODE_REVIEW_ENABLED=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-get workflow.code_review 2>/dev/null || echo "true")
```
...
2. Config value: `node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /iscape:code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
Spawn the iscape-code-reviewer agent:
...
No REVIEW.md created. You can retry with /iscape:code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /iscape:code-review ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  /iscape:code-review-fix ${PHASE_NUMBER}  — Auto-fix issues
