...
See ./references/checkpoints.md for complete checkpoint guidance.
</step>
...
For commit message conventions, see ./references/git-integration.md
</step>
...
- options:
  - "Review issues" - Analyze with /iscape:consider-issues
  - "Continue" - Address later, proceed to next work

**If "Review issues" selected:**
- Invoke: `SlashCommand("/iscape:consider-issues")`
...
**In YOLO mode:**
- Note issues were logged but don't prompt: `📋 [N] issues logged this phase (review later with /iscape:consider-issues)`
...
**Also available:**
- `/iscape:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/iscape:verify-work` — manual acceptance testing before completing milestone
- `/iscape:add-phase <description>` — add another phase before completing
