...
1. Check `context/STATE.md` for current milestone version
2. Check `context/milestones/` for the latest archived version
3. If neither found, check if `context/ROADMAP.md` exists (project may be mid-milestone)
4. If nothing found: error "No milestone found. Run /iscape:new-project or /iscape:new-milestone first."
...
**Archived milestone** (`context/milestones/v{VERSION}-ROADMAP.md` exists):
```
ROADMAP_PATH="context/milestones/v${VERSION}-ROADMAP.md"
REQUIREMENTS_PATH="context/milestones/v${VERSION}-REQUIREMENTS.md"
AUDIT_PATH="context/milestones/v${VERSION}-MILESTONE-AUDIT.md"
```

**Current/in-progress milestone** (no archive yet):
```
ROADMAP_PATH="context/ROADMAP.md"
REQUIREMENTS_PATH="context/REQUIREMENTS.md"
AUDIT_PATH="context/v${VERSION}-MILESTONE-AUDIT.md"
```

Note: The audit file moves to `context/milestones/` on archive (per `complete-milestone` workflow). Check both locations as a fallback.

**Always available:**
```
PROJECT_PATH="context/PROJECT.md"
RETRO_PATH="context/RETROSPECTIVE.md"
STATE_PATH="context/STATE.md"
```
...
```bash
iscape-tools.cjs init progress
```
...
Find the earliest `context/phases/` commit:
```bash
git log --oneline --diff-filter=A -- "context/phases/" | tail -1
```
...
Write to `context/reports/MILESTONE_SUMMARY-v${VERSION}.md`:
...
**Overwrite guard:** If `context/reports/MILESTONE_SUMMARY-v${VERSION}.md` already exists, ask the user:
...
```bash
iscape-tools.cjs commit "docs(v${VERSION}): generate milestone summary for onboarding" \
  --files "context/reports/MILESTONE_SUMMARY-v${VERSION}.md"
```
...
> "Summary written to `context/reports/MILESTONE_SUMMARY-v{VERSION}.md`.
>
...
If the user is done:
- Suggest next steps: `/iscape:new-milestone`, `/iscape:progress`, or sharing the summary with the team

## Step 9: Update STATE.md

```bash
iscape-tools.cjs state record-session \
  --stopped-at "Milestone v${VERSION} summary generated" \
  --resume-file "context/reports/MILESTONE_SUMMARY-v${VERSION}.md"
```
