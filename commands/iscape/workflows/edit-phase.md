...
Examples:
  `/iscape-edit-phase 5`       → phase = 5, force = false
  `/iscape-edit-phase 5 --force` → phase = 5, force = true
  `/iscape-edit-phase 12.1`    → phase = 12.1, force = false

If no argument provided:

```
ERROR: Phase number required
Usage: /iscape-edit-phase <phase-number> [--force]
Example: /iscape-edit-phase 5
Example: /iscape-edit-phase 5 --force
```
...
```bash
INIT=$(iscape-sdk query init.phase-op "${target}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Check `roadmap_exists` from init JSON. If false:
```
ERROR: No roadmap found (context/ROADMAP.md)
Run /iscape-new-project to initialize.
```
Exit.
</step>
...
```bash
PHASE_DATA=$(iscape-sdk query roadmap get-phase "${target}")
```
...
Available phases can be seen with /iscape-progress.
```
...
```bash
ANALYZE=$(iscape-sdk query roadmap analyze)
```
...
To edit anyway, run:
  /iscape-edit-phase {target} --force
```
...
```bash
ALL_PHASES=$(iscape-sdk query roadmap analyze)
```
...
```bash
iscape-sdk query state.add-roadmap-evolution \
  --phase {target} \
  --action edited \
...
- `/iscape-progress` — view updated roadmap
- `/iscape-plan-phase {target}` — re-plan this phase (if needed)
- `/iscape-discuss-phase {target}` — discuss implementation approach
