...
- Check for MILESTONE-CONTEXT.md (from /iscape:discuss-milestone)
...
```bash
node ./bin/iscape-tools.cjs commit "docs: start milestone v[X.Y] [Name]" --files context/PROJECT.md context/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node ./bin/iscape-tools.cjs init new-milestone)
```
...
**Persist choice to config** (so future `/iscape:plan-phase` honors it):

```bash
# If "Research first": persist true
node ./bin/iscape-tools.cjs config-set workflow.research true

# If "Skip research": persist false
node ./bin/iscape-tools.cjs config-set workflow.research false
```
...
Spawn 4 parallel iscape-project-researcher agents. Each uses this template with dimension-specific fields:
...
**Commit requirements:**
```bash
node ./bin/iscape-tools.cjs commit "docs: define milestone v[X.Y] requirements" --files context/REQUIREMENTS.md
```
...
- context/research/SUMMARY.md (if exists)
- context/config.json
...
`/iscape:discuss-phase [N]` — gather context and clarify approach
...
- [ ] iscape-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md phases continue from previous milestone
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/iscape:discuss-phase [N]`
