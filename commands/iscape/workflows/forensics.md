...
Post-mortem investigation for failed or stuck Iscape workflows. Analyzes git history,
`context/` artifacts, and file system state to detect anomalies and generate a
...
- `context/STATE.md` — current milestone, phase, progress, blockers, last session
- `context/ROADMAP.md` — phase list with status
- `context/config.json` — workflow configuration
...
For each phase directory in `context/phases/*/`:
...
Read `context/reports/SESSION_REPORT.md` if it exists — extract last session outcomes,
...
```bash
mkdir -p context/forensics
```

Write to `context/forensics/report-$(date +%Y%m%d-%H%M%S).md`:
...
3. {Recovery command if applicable — e.g., `/iscape:resume-work`, `/iscape:execute-phase N`}

---

*Report generated by `/iscape:forensics`. All paths redacted for portability.*
```
...
> "Report saved to `context/forensics/report-{timestamp}.md`.
>
...
iscape-tools.cjs state record-session \
  ...
  --resume-file "context/forensics/report-{timestamp}.md"
```
