...
2. **Sync chain flag with intent** — if user invoked manually (no `--auto`
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]] && [[ ! "$ARGUMENTS" =~ --chain ]]; then
     iscape-sdk query config-set workflow._auto_chain_active false || true
   fi
   ```

3. Read consolidated auto-mode (`active` = chain flag OR user preference):
   ```bash
   AUTO_MODE=$(iscape-sdk query check auto-mode --pick active 2>/dev/null || echo "false")
   ```
...
4. **If `--auto` or `--chain` flag present AND `AUTO_MODE` is not true:**
   ...
   ```bash
   iscape-sdk query config-set workflow._auto_chain_active true
   ```
...
5. **If `--auto` flag present OR `--chain` flag present OR `AUTO_MODE` is
   ...
   ```
   Skill(skill="iscape-plan-phase", args="${PHASE} --auto ${GSD_WS}")
   ```
...
6. **Handle plan-phase return:**
   ...
   - **PHASE COMPLETE** → Full chain succeeded. Display:
     ...
     Next: /iscape-discuss-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${GSD_WS}
     ```
   - **PLANNING COMPLETE** → Planning done, execution didn't complete:
     ```
     Auto-advance partial: Planning complete, execution did not finish.
     Continue: /iscape-execute-phase ${PHASE} ${GSD_WS}
     ```
   - **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
     ```
     Auto-advance stopped: Planning needs input.
     Continue: /iscape-plan-phase ${PHASE} ${GSD_WS}
     ```
   - **GAPS FOUND** → Stop chain:
     ```
     Auto-advance stopped: Gaps found during execution.
     Continue: /iscape-plan-phase ${PHASE} --gaps ${GSD_WS}
     ```
