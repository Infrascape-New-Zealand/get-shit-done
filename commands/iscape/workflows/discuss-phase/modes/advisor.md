...
```bash
ADVISOR_MODEL=$(iscape-sdk query resolve-model iscape-advisor-researcher --raw)
```
...
2. For EACH user-selected gray area, spawn a `Task()` in parallel:
   ...
   ```
   Task(
     prompt="First, read @~/.claude/agents/iscape-advisor-researcher.md for your role and instructions.
