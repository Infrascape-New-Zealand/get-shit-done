...
**Research-before-questions mode:** Check if `workflow.research_before_questions` is enabled in config (from init context or `context/config.json`). When enabled, before presenting questions for each area:
