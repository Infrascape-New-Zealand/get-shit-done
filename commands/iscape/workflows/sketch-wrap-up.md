...
UI implementation. Reads from `context/sketches/`, writes skill to `./.claude/skills/sketch-findings-[project]/`
(project-local) and summary to `context/sketches/WRAP-UP-SUMMARY.md`.
Companion to `/iscape-sketch`.
</purpose>
...
1. Read `context/sketches/MANIFEST.md` for the design direction and reference points
2. Glob `context/sketches/*/README.md` and parse YAML frontmatter from each
...
```
No unprocessed sketches found in `context/sketches/`.
Run `/iscape-sketch` first to create design explorations.
```
Exit.
...
1. Provide: `open context/sketches/NNN-name/index.html`
...
Write `context/sketches/WRAP-UP-SUMMARY.md` for project history:
...
```bash
iscape-sdk query commit "docs(sketch-wrap-up): package [N] sketch findings into project skill" --files context/sketches/WRAP-UP-SUMMARY.md
```
</step>
...
`/iscape-sketch` (run with no argument — its frontier mode analyzes the sketch landscape and proposes consistency and frontier sketches)

───────────────────────────────────────────────────────────────

**Also available:**
- `/iscape-plan-phase` — start building the real UI
- `/iscape-ui-phase` — generate a UI design contract for a frontend phase
- `/iscape-sketch [idea]` — sketch a specific new design area
...
- [ ] `context/sketches/WRAP-UP-SUMMARY.md` written for project history
- [ ] Project CLAUDE.md has auto-load routing line
- [ ] Summary presented
- [ ] Next-step options presented (including frontier sketch exploration via `/iscape-sketch`)
</success_criteria>
