...
iscape-plan-phase N → iscape-review N --codex → iscape-plan-phase N --reviews → iscape-review N --codex → ...
...
```bash
CONVERGENCE_ENABLED=$(iscape-sdk query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
```

**If `CONVERGENCE_ENABLED` is not `"true"`:** Display and exit:

```text
iscape-plan-review-convergence is disabled (workflow.plan_review_convergence=false).

This feature automates the plan→review→replan loop using external AI reviewers.
Enable it with:

  iscape config-set workflow.plan_review_convergence true

Then re-run: /iscape-plan-review-convergence {PHASE}
```
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► PLAN CONVERGENCE — Phase {phase_number}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Agent(
  ...
  prompt="Run /iscape-plan-phase for Phase {PHASE}.

Execute: Skill(skill='iscape-plan-phase', args='{PHASE} {GSD_WS}')
...
Agent(
  ...
  prompt="Run /iscape-review for Phase {PHASE}.

Execute: Skill(skill='iscape-review', args='--phase {PHASE} {REVIEWER_FLAGS} {GSD_WS}')
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

Display:
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► CONVERGENCE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Next: /iscape-execute-phase {PHASE}
```
...
To replan manually:  /iscape-plan-phase {PHASE} --reviews
To restart loop:     /iscape-plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
...
Agent(
  ...
  prompt="Run /iscape-plan-phase with --reviews for Phase {PHASE}.

Execute: Skill(skill='iscape-plan-phase', args='{PHASE} --reviews --skip-research {GSD_WS}')
...
IMPORTANT: When iscape-plan-phase outputs '## PLANNING COMPLETE', that means replanning is done. Return at that point.",
  mode="auto"
)
```
...
- [ ] Initial planning via Agent → Skill("iscape-plan-phase") if no plans exist
- [ ] Review via Agent → Skill("iscape-review") — isolated, not inline; {GSD_WS} forwarded
- [ ] Replan via Agent → Skill("iscape-plan-phase --reviews") — isolated, not inline
