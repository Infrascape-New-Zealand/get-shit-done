<purpose>
Remove a Iscape workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/iscape:list-workspaces` to show available workspaces, then ask:
