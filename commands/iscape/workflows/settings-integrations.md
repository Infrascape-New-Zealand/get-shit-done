<purpose>
Interactive configuration of third-party integrations for Iscape — search API keys
(Brave / Firecrawl / Exa), code-review CLI routing (`review.models.<cli>`), and
agent-skill injection (`agent_skills.<agent-type>`). Writes to
`context/config.json` via `iscape-sdk`/`iscape-tools` so unrelated keys are
preserved, never clobbered.

This command is deliberately separate from `/iscape-settings` (workflow toggles)
and any `/iscape-settings-advanced` tuning surface. It exists because API keys and
...
`context/config.json` — that is where secrets live on disk, and file
...
- **Plaintext is never echoed by AskUserQuestion descriptions, confirmation
  tables, or any log line.** It is not written to any file under `context/`
...
```bash
BRAVE=$(iscape-sdk query config-get brave_search --default null)
FIRECRAWL=$(iscape-sdk query config-get firecrawl --default null)
EXA=$(iscape-sdk query config-get exa_search --default null)
SEARCH_GITIGNORED=$(iscape-sdk query config-get search_gitignored --default false)
```
...
```bash
iscape-sdk query config-set brave_search "<value>"     # masked in output
iscape-sdk query config-set firecrawl "<value>"        # masked in output
iscape-sdk query config-set exa_search "<value>"       # masked in output
iscape-sdk query config-set search_gitignored true|false
```

For "Clear", write `null`:

```bash
iscape-sdk query config-set brave_search null
```
</step>
...
```bash
iscape-sdk query config-set review.models.<cli> "<command string>"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "iscape-executor", description: "Skills injected when spawning executor agents" },
      { label: "iscape-planner", description: "Skills injected when spawning planner agents" },
      { label: "iscape-verifier", description: "Skills injected when spawning verifier agents" },
...
```bash
iscape-sdk query config-set agent_skills.<slug> "<skill-a,skill-b,skill-c>"
```
...
Notes:
- API keys are stored plaintext in context/config.json. The confirmation
...
Quick commands:
- /iscape-settings — workflow toggles and model profile
- /iscape-set-profile <profile> — switch model profile
```
</step>
