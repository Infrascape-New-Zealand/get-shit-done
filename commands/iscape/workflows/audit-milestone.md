...
```bash
INIT=$(node ./bin/iscape-tools.cjs init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node ./bin/iscape-tools.cjs resolve-model iscape-integration-checker --raw)
```
...
node ./bin/iscape-tools.cjs phases list
```
...
PHASE_INFO=$(node ./bin/iscape-tools.cjs find-phase 01 --raw)
...
```bash
for summary in context/phases/*-*/*-SUMMARY.md; do
  node ./bin/iscape-tools.cjs summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
Create `context/v{version}-v{version}-MILESTONE-AUDIT.md` with:
...
**Report:** context/v{version}-MILESTONE-AUDIT.md
...
**Also available:**
- cat context/v{version}-MILESTONE-AUDIT.md — see full report
- /iscape:complete-milestone {version} — proceed anyway (accept tech debt)
