...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" init milestone-op)
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/iscape:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/iscape:new-milestone` first."
...
```bash
ROADMAP=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" roadmap analyze)
```
...
```bash
DETAIL=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```
Skill(skill="iscape:plan-phase", args="${PHASE_NUM}")
```
...
```
Skill(skill="iscape:execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="iscape:plan-phase", args="${PHASE_NUM} --gaps")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `iscape:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
cat context/PROJECT.md 2>/dev/null
cat context/REQUIREMENTS.md 2>/dev/null
...
```bash
find context/phases -name "*-CONTEXT.md" 2>/dev/null | sort
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="iscape:audit-milestone")
```

After audit completes, detect the result:

```bash
AUDIT_FILE="context/v${milestone_version}-MILESTONE-AUDIT.md"
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /iscape:audit-milestone to review, then /iscape:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /iscape:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="iscape:complete-milestone", args="${milestone_version}")
```

After complete-milestone returns, verify it produced output:

```bash
ls context/milestones/v${milestone_version}-ROADMAP.md 2>/dev/null
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /iscape:autonomous --from {next_phase}
```
...
- [ ] Audit invoked via Skill(skill="iscape:audit-milestone")
