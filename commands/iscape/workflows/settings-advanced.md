<purpose>
Interactive configuration of Iscape power-user knobs — plan bounce, node repair, subagent timeouts,
...
This is a companion to `/iscape-settings` — the common-case prompt there covers model profile,
...
```bash
iscape-sdk query config-ensure-section
if [[ -z "${GSD_CONFIG_PATH:-}" ]]; then
  if [[ -f context/active-workstream ]]; then
    WS=$(tr -d '\n\r' < context/active-workstream)
    GSD_CONFIG_PATH="context/workstreams/${WS}/config.json"
  else
    GSD_CONFIG_PATH="context/config.json"
  fi
fi
```

All subsequent reads and writes go through `$GSD_CONFIG_PATH`. Never hardcode
`context/config.json` — workstream installs must route to their own config file.
</step>
...
- `git.phase_branch_template` (default: `iscape/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `iscape/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or iscape/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or iscape/{milestone}-{slug}>)",
...
AskUserQuestion([
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Add --no-ignore to broad searches (includes context/)." }
    ]
  },
  {
...
This section lets the user inspect and override the built-in model IDs Iscape resolves for each
...
`iscape-sdk query config-set runtime <value>` before proceeding to Step C.
...
```bash
iscape-sdk query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
iscape-sdk query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Apply each selected value via `iscape-sdk query config-set <key> <value>` so the central
...
iscape-sdk query config-set workflow.plan_bounce_passes 5
iscape-sdk query config-set workflow.subagent_timeout 900
iscape-sdk query config-set git.base_branch main
iscape-sdk query config-set context_window 1000000
# Runtime model tier examples:
iscape-sdk query config-set runtime gemini
iscape-sdk query config-set model_profile_overrides.gemini.opus gemini-3-ultra
iscape-sdk query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `iscape-sdk query config-set` so sibling preservation is handled by
the central setter.
</step>
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► ADVANCED SETTINGS UPDATED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
These settings apply to future /iscape-plan-phase, /iscape-execute-phase, /iscape-discuss-phase,
and /iscape-ship runs.

For common-case toggles (model profile, research/plan_check/verifier, branching strategy,
UI/AI phase gates), use /iscape-settings.
```
</step>
...
- [ ] Writes routed through `iscape-sdk query config-set` so unrelated keys are preserved
