...
```bash
node ./bin/iscape-tools.cjs roadmap get-phase "${phase_number}"
grep -E "^| ${phase_number}" context/REQUIREMENTS.md 2>/dev/null
...
Use iscape-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ./bin/iscape-tools.cjs frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ./bin/iscape-tools.cjs roadmap get-phase "${phase_number}" --raw)
```
...
Use iscape-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ./bin/iscape-tools.cjs verify artifacts "$plan")
...
Use iscape-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ./bin/iscape-tools.cjs verify key-links "$plan")
