...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via iscape-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  Iscape > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /iscape:import --from <path>

  --from <path>   Import an external plan file into Iscape format
```
...
1. Read `context/ROADMAP.md` — extract phase structure, phase numbers, dependencies
2. Read `context/PROJECT.md` — extract project constraints, tech stack, scope boundaries.
   **If PROJECT.md does not exist:** skip constraint checks that rely on it and display:
   ```
   Iscape > Note: No PROJECT.md found. Conflict checks against project constraints will be skipped.
   ```
3. Read `context/REQUIREMENTS.md` — extract existing requirements for overlap and contradiction checks.
...
4. Glob for all CONTEXT.md files across phase directories:
   ```bash
   find context/phases/ -name "*-CONTEXT.md" -o -name "CONTEXT.md" 2>/dev/null
   ```
...
- **Iscape PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Display:
```
Iscape > BLOCKED: {N} blockers must be resolved before import can proceed.
```
...
Convert the imported content to Iscape PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to Iscape `{NN}-{MM}-PLAN.md` convention during conversion.

Apply Iscape naming convention for the output filename:
...
Delegate validation to iscape-plan-checker:

```
Task({
  subagent_type: "iscape-plan-checker",
  prompt: "Validate: context/phases/{phase}/{plan}-PLAN.md — check frontmatter completeness, task structure, and Iscape conventions. Report any issues."
})
```
...
Update `context/ROADMAP.md` to reflect the new plan:
- Add the plan to the Plans list under the correct phase section
- Include the plan name and description

Update `context/STATE.md` if appropriate (e.g., increment total plan count).

Commit the imported plan and updated files:
```bash
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" commit "docs({phase}): import plan from {basename FILEPATH}" --files context/phases/{phase}/{plan}-PLAN.md context/ROADMAP.md
```

Display completion:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Iscape ► IMPORT COMPLETE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `iscape-plan-checker` and `iscape-planner`
- Write `context/.active-skill` — this is a PBR pattern with no Iscape equivalent
