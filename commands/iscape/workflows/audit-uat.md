...
```bash
AUDIT=$(node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" audit-uat --raw)
```
...
1. **Close stale items:** `/iscape:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/iscape:verify-work {phase}`
```
</step>
