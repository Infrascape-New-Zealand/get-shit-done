...
Saves artifacts to `context/spikes/`. Companion to `/iscape-spike-wrap-up`.
...
If no `context/spikes/` directory exists, tell the user there's nothing to analyze and offer to start fresh with an idea instead.
...
**c. CONVENTIONS.md** — read `context/spikes/CONVENTIONS.md` if it exists. Established stack and patterns.

**d. All spike READMEs** — read `context/spikes/*/README.md` for verdicts, results, investigation trails, and tags.
...
Present all integration and frontier candidates, then ask which to run. When the user picks spikes, write definitions into `context/spikes/MANIFEST.md` (appending to existing table) and proceed directly to building them starting at `research`.
</step>

<step name="setup_directory">
Create `context/spikes/` if it doesn't exist:

```bash
mkdir -p context/spikes
```

Check for existing spikes to determine numbering:
```bash
ls -d context/spikes/[0-9][0-9][0-9]-* 2>/dev/null | sort | tail -1
```

Check `commit_docs` config:
```bash
COMMIT_DOCS=$(iscape-sdk query config-get commit_docs 2>/dev/null || echo "true")
```
</step>
...
**Check conventions first.** If `context/spikes/CONVENTIONS.md` exists, follow its stack and patterns — these represent validated choices the user expects to see continued.
...
If `context/spikes/` has existing content, load context in this priority order:

**a. Conventions:** Read `context/spikes/CONVENTIONS.md` if it exists.

**b. Findings skills:** Glob for `./.claude/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `references/*.md` files.

**c. Manifest:** Read `context/spikes/MANIFEST.md` for the index of all spikes.

**d. Related READMEs:** Based on the new idea, identify which prior spikes are related by matching tags, names, technologies, or domain overlap. Read only those `context/spikes/*/README.md` files. Skip unrelated ones.
...
If no `context/spikes/` exists, skip this step.
</step>
...
Create or update `context/spikes/MANIFEST.md`:
...
Before starting each spike (not just the first), re-read `context/spikes/MANIFEST.md` and `context/spikes/CONVENTIONS.md` to prevent drift within long sessions. Check the Requirements section — make sure the spike doesn't contradict any established requirements.
</step>
...
**a.** Create `context/spikes/NNN-descriptive-name/`
...
**h.** Update `context/spikes/MANIFEST.md` with the spike's row.

**i.** Commit (if `COMMIT_DOCS` is true):
```bash
iscape-sdk query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files context/spikes/NNN-descriptive-name/ context/spikes/MANIFEST.md
```
...
After all spikes in this session are built, update `context/spikes/CONVENTIONS.md` with patterns that emerged or solidified.
...
```bash
iscape-sdk query commit "docs(spikes): update conventions" --files context/spikes/CONVENTIONS.md
```
</step>
...
`/iscape-spike-wrap-up`

───────────────────────────────────────────────────────────────

**Also available:**
- `/iscape-spike` — spike more ideas (or run with no argument for frontier mode)
- `/iscape-plan-phase` — start planning the real implementation
- `/iscape-explore` — continue exploring the idea
...
<success_criteria>
- [ ] `context/spikes/` created (auto-creates if needed, no project init required)
