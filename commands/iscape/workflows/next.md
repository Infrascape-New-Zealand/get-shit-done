<purpose>
Detect current project state and automatically advance to the next logical Iscape workflow step.
...
```bash
# Get state snapshot
node "$HOME/.claude/get-shit-done/bin/iscape-tools.cjs" state json 2>/dev/null || echo "{}"
```

Also read:
- `context/STATE.md` — current phase, progress, plan counts
- `context/ROADMAP.md` — milestone structure and phase list
...
```
No Iscape project detected. Run `/iscape:new-project` to get started.
```
Exit.
</step>
...
→ Next action: `/iscape:discuss-phase <first-phase>`
...
▶ **Next step:** `/iscape:[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/iscape:next` is zero-friction advancement.
</step>
