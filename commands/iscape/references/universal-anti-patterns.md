...
3. **Read depth scales with context window** -- check `context_window_tokens` in `context/config.json`. At < 500000: read only frontmatter, status fields, or summaries. At >= 500000 (1M model): full body reads permitted when content is needed for inline decisions. See `references/context-budget.md` for the complete table.
...
8. **Never** read `context/logs/` files -- only the health workflow reads these.
...
10. **Dispatch subagents via `subagent_type: "general-purpose"`** and embed the iscape agent role (e.g. phase-researcher, executor, planner) in the prompt itself. Iscape does not register custom agent types -- role instructions live in `commands/iscape/agents/iscape-*.md` and should be inlined into the dispatch prompt so the general-purpose agent behaves as the intended iscape role.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `iscape-tools.cjs` CLI commands (`state update`, `state advance-plan`, `roadmap update-status`) for mutations. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
## Iscape-Specific Rules

24. **Do not** check for `mode === 'auto'` or `mode === 'autonomous'` -- Iscape uses `yolo` config flag. Check `yolo: true` for autonomous mode, absence or `false` for interactive mode.
25. **Always use `iscape-tools.cjs`** (not `iscape-tools.js` or any other variant) -- Iscape uses CommonJS for Node.js CLI compatibility.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- iscape-tools detection depends on this exact pattern.
