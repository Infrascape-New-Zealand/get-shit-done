...
The `--ws <name>` flag scopes Iscape operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `ISCAPE_WORKSTREAM` environment variable (per-instance)
3. Session-scoped active workstream pointer in temp storage (per runtime session / terminal)
4. `context/active-workstream` file (legacy shared fallback when no session key exists)
5. `null` — flat mode (no workstreams)

## Why session-scoped pointers exist

The shared `context/active-workstream` file is fundamentally unsafe when multiple
...
Iscape now prefers a session-scoped pointer keyed by runtime/session identity
(`ISCAPE_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When Iscape resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `ISCAPE_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, Iscape does not keep probing. It falls
back directly to the legacy shared `context/active-workstream` file.

This matters in headless or stripped environments: when stdin is already
non-interactive, Iscape intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, Iscape also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, Iscape
...
Iscape does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${ISCAPE_WS}` which:
...
```bash
# All iscape-tools commands accept --ws
node iscape-tools.cjs state json --ws feature-a
node iscape-tools.cjs find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
ISCAPE_SESSION_KEY=my-terminal-a node iscape-tools.cjs workstream set feature-a
ISCAPE_SESSION_KEY=my-terminal-a node iscape-tools.cjs state json
ISCAPE_SESSION_KEY=my-terminal-b node iscape-tools.cjs workstream set feature-b
ISCAPE_SESSION_KEY=my-terminal-b node iscape-tools.cjs state json

# Workstream CRUD
node iscape-tools.cjs workstream create <name>
node iscape-tools.cjs workstream list
node iscape-tools.cjs workstream status <name>
node iscape-tools.cjs workstream complete <name>
```
