...
3. **Read depth scales with context window** -- check `context_window_tokens` in `context/config.json`:
...
**How to check:** Read `context/config.json` and inspect `context_window_tokens`. If the field is absent, treat as 200k (conservative default).
